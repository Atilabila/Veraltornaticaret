...
// Load local env like Next.js does in dev (minimal).
...
async function main() {
  // Load from .env.local when running via: tsx -r dotenv/config scripts/verify-product-variants.ts
  ...
  // 1) Does table exist / readable?
  const head = await (supabase.from('product_variants') as any).select('id', { head: true, count: 'exact' });
  if (head.error) {
  ...
  // 2) Relationship works? (PostgREST foreign key link)
  const rel = await (supabase.from('metal_products') as any)
