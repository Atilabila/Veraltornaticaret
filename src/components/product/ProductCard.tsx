...
const ProductCard: React.FC<ProductCardProps> = ({ product, variant = "default" }) => {
  ...
  const header = (
    <div
      className={`flex justify-between items-center ${
        isHorizontal ? "px-4 pt-4 pb-2 border-b" : "p-4 border-b"
  ...
  const media = (
    <Link
      ...
      className={`relative ${
        isHorizontal
          ? "w-24 h-24 sm:w-28 sm:h-28 flex-shrink-0 lg:w-auto lg:h-auto lg:aspect-square"
          : "aspect-square"
      } overflow-hidden bg-black block group-image`}
    >
      {product.image_url ? (
        <img
          src={normalizeImagePath(product.image_url)}
          alt={product.name}
          className={`object-contain w-full h-full transition-transform duration-700 group-hover:scale-105 opacity-80 group-hover:opacity-100 ${
            isHorizontal ? "p-3" : "p-8"
          }`}
        />
      ) : (
  ...
  const content = (
    <div
      className={`flex flex-col flex-grow ${isHorizontal ? "p-4 gap-3" : "p-6 space-y-6"} relative ${
    ...
    >
      <div className="space-y-2">
        <div className="flex flex-col gap-1">
          ...
          <h3
            className={`font-black uppercase italic group-hover:text-[#D4AF37] transition-colors text-white tracking-tighter line-clamp-2 ${
              isHorizontal
                ? "text-base sm:text-lg leading-tight break-words max-w-[22ch] sm:max-w-none"
                : "text-xl leading-none min-h-[2.5rem]"
            }`}
          >
            {product.name}
          </h3>
      ...
      <p className="text-xs text-white/40 line-clamp-3 font-medium leading-relaxed font-mono pr-1">
        {product.description ||
        ...
        <div className="flex flex-col">
          {isRetail ? (
            <>
              ...
              <span className={`font-black ${isHorizontal ? "text-xl" : "text-2xl"} text-white tracking-tighter`}>
  ...
  return (
    <div
      className={`group relative flex ${isHorizontal ? "flex-row gap-4" : "flex-col"} h-full bg-[#111827] border border-white/5 transition-all duration-300 hover:border-[#D4AF37]/50 shadow-lg hover:shadow-[0_0_30px_rgba(212,175,55,0.1)]`}
